...
```{r}
median_age_variables <- acs_vars_2022 %>%
  filter(str_detect(label, "[Mm]edian.*age"))
median_age_concept <- acs_vars_2022 %>%
...
ggplot(county_data) + 
  aes(x=total_popE) +
  geom_histogram(fill = "green", width = 0.5) +
  labs(
...
ggplot(county_data) +
  aes(x=total_popE) + 
  geom_boxplot(fill = "purple", width = 0.2) + 
  labs(title = "Population Distribution")
```
```{r}
top_three_counties <- county_data %>%
  arrange(desc(total_popE)) %>%
  slice_head(n=3)
```
 Top three counties are Philadelphia, Allegheny, and Montgomery
------------------------------------------------------------------------
...
ggplot(county_data)+
  ...
  geom_point(color = "red", alpha = 0.5) + 
    geom_smooth(method = "lm", se = TRUE, color = "red") +
  labs(
    title = "Median age vs. Median Income for PA Counties",
    subtitle = "2018-2022 ACS 5-Year Estimates",
    x = "Median Age",
    y = "Median Household Income ($)",
    caption = "Source: U.S. Census Bureau"
